...
import { fetchFilterProducts, getTelegramIdForApi } from "@/lib/api";
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-6 h-auto sm:h-[500px]">
            ...
            <div className="sm:w-3/12 flex sm:flex-col gap-2 sm:gap-3">
              {[...Array(4)].map((_, i) => (
                <Skeleton
                  key={i}
                  className="w-16 h-16 sm:w-full sm:h-[100px]"
                />
              ))}
            </div>
          </div>
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <div className="relative w-full h-full cursor-zoom-in">
                    <Image
                      ...
                      src={
                        mainImage ||
                        "/placeholder.svg?height=350&width=350&query=product image"
                      }
                ...
                <DialogContent className="max-w-4xl p-0 bg-transparent border-none shadow-none">
                  ...
                  <Image
                    ...
                    src={
                      mainImage ||
                      "/placeholder.svg?height=500&width=500&query=product image"
                    }
                    alt="zoomed"
                    ...
                    {filteredImages.map((img: any, index: number) => (
                      <SwiperSlide key={index}>
                        <div
                          ...
                          onClick={() =>
                            handleThumbnailClick(img?.image, index)
                          }
                        >
                          <Image
                            src={
                              img.image ||
                              "/placeholder.svg?height=60&width=60&query=thumbnail"
                            }
                            alt={`Thumbnail ${index}`}
                            width={60}
                            height={60}
                  ...
                  {filteredImages.map((img: any, index: number) => (
                    ...
                    >
                      <Image
                        src={
                          img.image ||
                          "/placeholder.svg?height=90&width=90&query=thumbnail"
                        }
                        alt={`Thumbnail ${index}`}
                        width={90}
                        height={90}
                    ...
                    <DialogContent className="min-w-[100%-10px] lg:min-w-[800px] mx-auto">
                      ...
                      <div className="py-0">
                        <Image
                          ...
                          src={
                            sizeImage ||
                            "/placeholder.svg?height=100&width=100&query=size chart"
                          }
                          alt="size image"
                        />
                ...
                <div className="flex gap-2 flex-wrap">
                  {availableSizes.map((size: any, idx: number) => {
                    const isSizeAvailable = selectedColor
                      ...
                      : product.variants.some(
                          (variant: any) =>
                            variant.size_name === size.name && variant.stock > 0
                        );
                ...
                <div className="mt-3 p-3 bg-gray-50 rounded-lg border">
                  ...
                  <div className="space-y-1 text-xs text-gray-600">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        Режим работы: {selectedBranchDetails.working_hours}
                      </span>
                    </div>
                    {selectedBranchDetails.has_fitting_room && (
                      <p className="text-green-600">✓ Примерочная доступна</p>
                    )}
                    {selectedBranchDetails.has_parking && (
                      <p className="text-green-600">✓ Парковка доступна</p>
                    )}
